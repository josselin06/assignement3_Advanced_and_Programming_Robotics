...
//main loop
    ...
    // Get the parameters
        ...
        if (n > 0) {
            ...
            sscanf(buffer, "%f %f %f %f %f %d %f %f %f %f %f %f %c %f %f",
    ...
    //----------------Start calculation-------------------------------------------
        ...
        */
        ...
        float distance=sqrt((Xdrone-Xobst)*(Xdrone-Xobst)+(Ydrone-Yobst)*(Ydrone-Yobst)); //absolute distance between the obstacle and the drone
        if(distance<repR)
        {
            float repulsive_force = repF *exp(-distance/repR); //Repulsive force of the obstacle
            float theta = atan2(Ydrone - Yobst,Xdrone - Xobst); //angle trigonometric angle between the obsttacle and the drone
